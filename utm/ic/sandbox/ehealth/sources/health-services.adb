package body Health.Services is
 ...
 ------ Project_Weight_----
 -------------------------
 function  Project_Weight (Person : in Interfaces.Human'Class) return Mass_Type
 is
  Calorie_Consume       : Calorie_Type  := 2000.0 * cal;
  Daily_Calorie_Intake  : Calorie_Type  := 0.0 * cal;
  Excess_Calorie_Intake : Calorie_Type  := 0.0 * cal;
  Weight_Gain           : Mass_Type     := 0.0 * kg;
 begin

  Daily_Calorie_Intake := DTEE (Activity => Person.Activity,
                                Mass     => Person.Weight,
  ...
  if Calorie_Consume > Daily_Calorie_Intake then
   Excess_Calorie_Intake :=  (Calorie_Consume - Daily_Calorie_Intake) * 365.0;
     Weight_Gain := (Excess_Calorie_Intake/7700.0) * mr;
